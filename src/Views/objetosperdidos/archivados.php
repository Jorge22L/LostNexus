<div class="bg-(--color-bg)/10 py-6">
    <div class="container mx-auto px-4">
        <h2 class="text-(--color-secondary) font-bold text-2xl text-center">Objetos Archivados</h2>
        <p class="text-center text-gray-600">Objetos con más de 2 meses de antigüedad.</p>
    </div>
</div>
...
<section class="bg-(--color-bg)/10 py-12">
    <div class="container mx-auto px-4">
        ...
        if (isset($_SESSION['alerta'])) {
            ...
            $tipo = $alerta['tipo'] === 'exito' ? 'green' : 'red';
            echo "<div class='bg-{$tipo}-100 border border-{$tipo}-400 text-{$tipo}-700 px-4 py-3 rounded relative mb-4' role='alert'>";
            echo "<span class='block sm:inline'>{$alerta['mensaje']}</span>";
            echo '</div>';
    ...
    <div class="container mx-auto px-4">
        <form class="mb-8" method="GET" action="/objetosperdidos/archivados">
            <!-- Filtros -->
            <div class="mb-8">
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full">
                    ...
                    <button type="submit" class="flex-1 sm:flex-none px-4 py-2 bg-(--color-bg) text-white rounded-lg hover:bg-(--color-secondary) transition-colors flex items-center justify-center gap-2">
        ...
        <?php if (empty($objetos)): ?>
            <div class="bg-white rounded-lg shadow p-8 text-center">
                ...
                <h3 class="mt-4 text-lg font-medium text-gray-900">No se encontraron objetos archivados</h3>
                <p class="mt-2 text-gray-600">
                    <?php echo !empty($nombre_filtro) || !empty($categoria_filtro) || !empty($fecha_filtro) ? 
                        'No hay objetos archivados que coincidan con tus criterios de búsqueda.' : 
                        'No hay objetos con más de 2 meses de antigüedad.'; ?>
                </p>
                <?php if (!empty($nombre_filtro) || !empty($categoria_filtro) || !empty($fecha_filtro)): ?>
                    <div class="mt-6">
                        <a href="/objetosperdidos/archivados" class="inline-flex items-center px-4 py-2 bg-(--color-bg) border border-transparent rounded-md font-semibold text-white hover:bg-(--color-secondary) focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-(--color-bg) transition-colors">
                            Limpiar filtros
                        </a>
                    </div>
                <?php endif; ?>
        ...
        <?php else: ?>
            ...
            <?php if ($totalPaginas > 1): ?>
                <div class="flex items-center justify-center mt-8 space-x-2">
                    <a href="/objetosperdidos/archivados?page=<?php echo max(1, $paginaActual - 1); ?>&nombre=<?php echo urlencode($nombre_filtro ?? ''); ?>&categoria=<?php echo urlencode($categoria_filtro ?? ''); ?>&fecha=<?php echo urlencode($fecha_filtro ?? ''); ?>"
                    ...
                    for ($i = $inicio; $i <= $fin; $i++): ?>
                        <a href="/objetosperdidos/archivados?page=<?php echo $i; ?>&nombre=<?php echo urlencode($nombre_filtro ?? ''); ?>&categoria=<?php echo urlencode($categoria_filtro ?? ''); ?>&fecha=<?php echo urlencode($fecha_filtro ?? ''); ?>"
                    ...
                    <a href="/objetosperdidos/archivados?page=<?php echo min($totalPaginas, $paginaActual + 1); ?>&nombre=<?php echo urlencode($nombre_filtro ?? ''); ?>&categoria=<?php echo urlencode($categoria_filtro ?? ''); ?>&fecha=<?php echo urlencode($fecha_filtro ?? ''); ?>"
