<div class="bg-(--color-bg)/10 py-6">
    <div class="container mx-auto px-4">
        <h2 class="text-(--color-secondary) font-bold text-2xl text-center mb-2">Listado de Categorías</h2>
        <p class="text-center text-(--color-secondary)/80 max-w-2xl mx-auto">Administra las categorías disponibles para clasificar los objetos perdidos</p>
    </div>
</div>

<section class="bg-white py-8 px-4 sm:px-6 lg:px-8">
    <div class="container mx-auto">
        <!-- Barra de herramientas (añadir nueva categoría) -->
        <form method="get" action="/categorias" class="flex justify-between items-center mb-6">
            ...
            <a href="/agregar_categoria" class="bg-(--color-bg) hover:bg-(--color-secondary) text-white px-4 py-2 rounded-lg flex items-center transition-colors">
                ...
                <div class="p-8 text-center">
                    ...
                    <p class="mt-2 text-gray-600">
                        <?php echo !empty($busqueda) ?
                            ...
                            'Actualmente no hay categorías registradas.' ?>
                    </p>
                    <div class="mt-6">
                        <?php if (!empty($busqueda)): ?>
                            <a href="/categorias" class="inline-flex items-center px-4 py-2 bg-(--color-bg) border border-transparent rounded-md font-semibold text-white hover:bg-(--color-secondary) focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-(--color-bg) transition-colors">
                                Mostrar todas
                            </a>
                        <?php else: ?>
                            <a href="/agregar_categoria" class="inline-flex items-center px-4 py-2 bg-(--color-bg) border border-transparent rounded-md font-semibold text-white hover:bg-(--color-secondary) focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-(--color-bg) transition-colors">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php foreach ($categorias as $categoria) : ?>
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?php echo $categoria->id; ?></td>
                                <td class="px-6 py-4 whitespace-nowrap text-md text-gray-700"><?php echo htmlspecialchars($categoria->nombre); ?></td>
                                <td class="px-6 py-4 whitespace-nowrap text-md text-gray-700">
                                    <div class="flex space-x-3">
                                        <a href="/categorias/editar/<?php echo $categoria->id; ?>" class="text-(--color-bg) hover:text-(--color-secondary) p-1 rounded hover:bg-(--color-bg)/10 transition-colors">
                                        ...
                                        <dialog class="fixed top-0 left-0 w-full h-full z-50 flex items-center justify-center p-4 bg-transparent bg-opacity-50 backdrop:bg-gray-600/50 opacity-0 invisible transition-opacity duration-300 open:opacity-100 open:visible">
                                            <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full mx-auto transform transition-all">
                                                <div class="p-6">
                                                    ...
                                                    <div class="mt-4">
                                                        <p class="text-(--color-secondary)/80">¿Estás seguro de eliminar la categoría "<?php echo htmlspecialchars($categoria->nombre); ?>"?</p>
                                                    </div>
                ...
                <?php if ($totalPaginas > 1): ?>
                    <div class="bg-white px-6 py-3 flex items-center justify-between border-t border-gray-200">
                        ...
                        <div class="flex-1 flex justify-between sm:hidden">
                            <a href="/categorias?page=<?php echo max(1, $paginaActual - 1); ?>&busqueda=<?php echo urlencode($busqueda ?? ''); ?>"
                            ...
                            <a href="/categorias?page=<?php echo min($totalPaginas, $paginaActual + 1); ?>&busqueda=<?php echo urlencode($busqueda ?? ''); ?>"
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700">
                                    ...
                                    <span class="font-medium"><?php echo $totalCategorias; ?></span> resultados
                                </p>
                            </div>
                            ...
                            <div>
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                    ...
                                    if ($inicioPaginas > 1): ?>
                                        <a href="/categorias?page=1&busqueda=<?php echo urlencode($busqueda ?? ''); ?>" class="bg-white border-gray-300 text-gray-500 hover:bg-gray-50 relative inline-flex items-center px-4 py-2 border text-sm font-medium">
                                            1
                                        </a>
                                    ...
                                    <?php for ($i = $inicioPaginas; $i <= $finPaginas; $i++): ?>
                                        <a href="/categorias?page=<?php echo $i; ?>&busqueda=<?php echo urlencode($busqueda ?? ''); ?>"
                                    ...
                                    <?php if ($finPaginas < $totalPaginas): ?>
                                        ...
                                        <?php endif; ?>
                                        <a href="/categorias?page=<?php echo $totalPaginas; ?>&busqueda=<?php echo urlencode($busqueda ?? ''); ?>" class="bg-white border-gray-300 text-gray-500 hover:bg-gray-50 relative inline-flex items-center px-4 py-2 border text-sm font-medium">
