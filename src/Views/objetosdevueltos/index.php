<div class="bg-(--color-bg)/10 py-6">
    <div class="container mx-auto px-4">
        <h2 class="text-(--color-secondary) font-bold text-2xl text-center">Historial de Objetos Devueltos</h2>
    </div>
</div>

<section class="bg-(--color-bg)/10 py-12">
    <div class="container mx-auto px-4">
        <form class="mb-8" method="GET" action="/objetosdevueltos">
            <!-- Filtros -->
            <div class="mb-8">
                <!-- Búsqueda con icono -->
        ...
        <?php else: ?>
                ...
                <?php foreach ($objetos as $objeto): ?>
                    <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div class="h-48 overflow-hidden bg-gray-100 flex items-center justify-center">
                            <img class="h-full w-full object-cover hover:scale-105 transition-all duration-500" src="/imagenes/<?php echo $objeto->foto; ?>" alt="<?php echo $objeto->nombre; ?>">
                        </div>
                        <div class="p-4">
                            ...
                            <div class="flex justify-between items-center mt-4">
                                <a href="/objetosdevueltos/ver/<?php echo $objeto->id; ?>" class="bg-(--color-primary) text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors hover:bg-(--color-primary)/80">Ver detalles</a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
            ...
            <?php if ($totalPaginas > 1): ?>
                <div class="flex items-center justify-center mt-8 space-x-2">
                    <!-- Anterior -->
                    <a href="/objetosdevueltos?page=<?php echo max(1, $paginaActual - 1); ?>&nombre=<?php echo urlencode($nombre_filtro ?? ''); ?>&categoria=<?php echo urlencode($categoria_filtro ?? ''); ?>&fecha=<?php echo urlencode($fecha_filtro ?? ''); ?>"
                    ...
                    for ($i = $inicio; $i <= $fin; $i++): ?>
                        <a href="/objetosdevueltos?page=<?php echo $i; ?>&nombre=<?php echo urlencode($nombre_filtro ?? ''); ?>&categoria=<?php echo urlencode($categoria_filtro ?? ''); ?>&fecha=<?php echo urlencode($fecha_filtro ?? ''); ?>"
                    ...
                    <!-- Siguiente -->
                    <a href="/objetosdevueltos?page=<?php echo min($totalPaginas, $paginaActual + 1); ?>&nombre=<?php echo urlencode($nombre_filtro ?? ''); ?>&categoria=<?php echo urlencode($categoria_filtro ?? ''); ?>&fecha=<?php echo urlencode($fecha_filtro ?? ''); ?>"
